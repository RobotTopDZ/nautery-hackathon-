...
interface SelectedFeature {
  type: 'station' | 'zone' | 'site' | 'general'
  data: WastewaterStation | PollutionZone | EnvironmentalSite | any
}
...
export function ToulonInteractiveMap({ className }: ToulonInteractiveMapProps) {
    ...
    import('leaflet').then((L) => {
      ...
      map.on('click', function(e) {
        ...
        setSelectedFeature({
          type: 'general',
          data: {
            name: 'Point sur la carte',
            coordinates: [latlng.lat, latlng.lng],
            description: `Coordonnées: ${latlng.lat.toFixed(4)}, ${latlng.lng.toFixed(4)}`
          }
        })
      })
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      // Clear all layers
      Object.values(layerGroupsRef.current).forEach((group: any) => {
        group.clearLayers()
      })
      ...
      // Add pollution zones with strong visual impact
      if (showZones) {
        toulonPollutionZones.forEach((zone) => {
          const color = getZoneColor(zone.pollutionLevel)
          
          // Create multiple circles for strong gradient effect
          const radiusSteps = [
            { radius: zone.radius * 1000 * 2.0, opacity: 0.05 },
            { radius: zone.radius * 1000 * 1.7, opacity: 0.1 },
            { radius: zone.radius * 1000 * 1.4, opacity: 0.15 },
            { radius: zone.radius * 1000 * 1.2, opacity: 0.25 },
            { radius: zone.radius * 1000, opacity: 0.4 },
            { radius: zone.radius * 1000 * 0.7, opacity: 0.55 },
            { radius: zone.radius * 1000 * 0.4, opacity: 0.7 }
          ]
          ...
          radiusSteps.forEach((step, index) => {
            const circle = L.circle(zone.center, {
              ...
              weight: index === radiusSteps.length - 1 ? 3 : 0,
            ...
            layerGroupsRef.current.zones.addLayer(circle)
          })
          ...
          layerGroupsRef.current.zones.addLayer(zoneMarker)
        })
      }

      // Add wastewater stations
      if (showStations) {
        toulonStations.forEach((station) => {
          const stationColor = getZoneColor(station.pollutionLevel)
          
          // Impact zone around rejection point with gradient
          const impactSteps = [
            { radius: station.impactRadius * 1000 * 1.5, opacity: 0.1 },
            { radius: station.impactRadius * 1000 * 1.2, opacity: 0.15 },
            { radius: station.impactRadius * 1000, opacity: 0.25 },
            { radius: station.impactRadius * 1000 * 0.6, opacity: 0.35 }
          ]

          impactSteps.forEach((step) => {
            const impactCircle = L.circle(station.rejectionPoint, {
              radius: step.radius,
            ...
            impactCircle.on('click', function(e) {
              L.DomEvent.stopPropagation(e)
              setSelectedFeature({ type: 'station', data: station })
            })

            layerGroupsRef.current.stations.addLayer(impactCircle)
          })

          // Connection line between station and rejection point
          const connectionLine = L.polyline([station.coordinates, station.rejectionPoint], {
            color: stationColor,
            weight: 4,
            opacity: 0.8,
            dashArray: '10, 10'
          ...
          connectionLine.on('click', function(e) {
            L.DomEvent.stopPropagation(e)
            setSelectedFeature({ type: 'station', data: station })
          })

          layerGroupsRef.current.stations.addLayer(connectionLine)
          ...
          const stationMarker = L.marker(station.coordinates, { icon: stationIcon })
            .bindPopup(`
          ...
          layerGroupsRef.current.stations.addLayer(stationMarker)
          ...
          layerGroupsRef.current.stations.addLayer(rejectionMarker)
        })
      }
      ...
      if (showSites) {
        toulonEnvironmentalSites.forEach((site) => {
          ...
          const siteMarker = L.marker(site.coordinates, { icon: siteIcon })
            .bindPopup(`
          ...
          layerGroupsRef.current.sites.addLayer(siteMarker)
        })
      }
    })
  }, [mapReady, showStations, showZones, showSites])
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2 p-3 bg-gray-800/50 rounded-lg">
          ...
          >
            ...
            <span>Stations d'épuration ({toulonStations.length})</span>
          </Button>
          <Button
          ...
          >
            ...
            <span>Zones polluées ({toulonPollutionZones.length})</span>
          </Button>
          <Button
          ...
          >
            ...
            <span>Sites environnementaux ({toulonEnvironmentalSites.length})</span>
          </Button>
        </div>
          ...
          <div>
            ...
            <div className="space-y-1 text-xs">
              <div className="flex items-center space-x-2">
                ...
                <span>Critique (Port militaire)</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-orange-600"></div>
                <span>Élevée (Industrie, Commerce)</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full bg-yellow-600"></div>
                <span>Modérée (Urbain)</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-2">
                ...
                {selectedFeature.type === 'general' && <MapPin className="h-5 w-5 text-gray-400" />}
            ...
            <div className="space-y-2 text-sm">
              ...
              {selectedFeature.type === 'zone' && (
                <div className="space-y-2">
                  ...
                  <div><strong>Rayon:</strong> {(selectedFeature.data as PollutionZone).radius} km</div>
                </div>
              )}
              ...
              {selectedFeature.type === 'general' && (
                <div className="space-y-2">
                  <div><strong>Coordonnées:</strong> {selectedFeature.data.coordinates[0].toFixed(4)}, {selectedFeature.data.coordinates[1].toFixed(4)}</div>
                  <div className="text-xs text-neutral/70">Cliquez sur les zones colorées ou marqueurs pour plus d'informations</div>
                </div>
              )}
